...
<template>
    <div class="subject rounded-md" :class="`${formatSpaces(props.subjectObject.name)} subject_starts-${props.subjectObject.starts} subject_duration-${props.subjectObject.duration}`" :style="{ backgroundColor: props.subjectObject.color, borderColor: props.subjectObject.color }">
        <div class="subject-container flex flex-col items-start gap-1 relative p-[5px] md:p-[10px]">
            ...
            <div class="subject-time px-2 py-1 bg-[rgba(255,255,255,0.2)] rounded-md hidden md:inline-block">
                <p class="text-white">{{ `${parseInt(props.subjectObject.starts.split('_')[0]) < 10 ? '0' + props.subjectObject.starts.split('_')[0] : props.subjectObject.starts.split('_')[0] }:${parseInt(props.subjectObject.starts.split('_')[1]) < 10 ? '0' + props.subjectObject.starts.split('_')[1] : props.subjectObject.starts.split('_')[1]}` }}</p>
            </div>
            <div class="subject-menu absolute top-[5px] right-[5px]" @click="edit_subject_popup.editSubjectOpen(props.subjectObject)">
